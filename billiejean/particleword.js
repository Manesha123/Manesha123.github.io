    ...
    //the pixels are an array
    var pixels=new Array();
    //getting the element from html
    var canv=$('canv');
    //the canvas is 2d
    var ctx=canv.getContext('2d');
    //getting the word canvas from html
    var wordCanv=$('wordCanv');
    //the second canvas is 2d
    ...
    //width of teh canvas
    canv.width=cw;
    //height og the canvas
      canv.height=ch;
    //the size of the word canvas is the same as the normal canvas
      wordCanv.width=cw;
      wordCanv.height=ch;
...
function canv_mousemove(evt)
    ...
    function Pixel(homeX,homeY)
    {
      ...
      //tmp
      this.xVelocity=Math.random()*2-5;
    ...
    Pixel.prototype.move=function()
    {
      ...
      if(mx >= 0)
      {
        ...
        cursorForce=Math.min(1000/cursorDistanceSquared,10000);
